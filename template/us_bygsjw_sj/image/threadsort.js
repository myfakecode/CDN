...
function changeselectthreadsort(selectchoiceoptionid, optionid, type) {
	...
	byg_id('select_' + forum_optionlist[soptionid]['sidentifier']).innerHTML = selectoption;
}

function checkoption(identifier, required, checktype, checkmaxnum, checkminnum, checkmaxlength) {
	if(checktype != 'image' && checktype != 'select' && !byg_id('typeoption_' + identifier) || !byg_id('check' + identifier)) {
		return true;
	}

	var ce = byg_id('check' + identifier);
	ce.innerHTML = '';

	if(checktype == 'select') {
		if(required != '0' && (byg_id('typeoption_' + identifier) == null || byg_id('typeoption_' + identifier).value == '0')) {
			warning(ce, '必填项目没有填写');
			return false;
		} else if(required == '0' && (byg_id('typeoption_' + identifier) == null || byg_id('typeoption_' + identifier).value == '0')) {
	...
	if(checktype == 'radio' || checktype == 'checkbox') {
		var nodes = byg_id('typeoption_' + identifier).parentNode.parentNode.parentNode.getElementsByTagName('INPUT');
	...
	if(checktype == 'image') {
		var checkvalue = byg_id('sortaid_' + identifier).value;
	} else {
		var checkvalue = byg_id('typeoption_' + identifier).value;
	}
